...
APP_VERSION = "2026-05-01-buyer-trader-visible-impact"
    ...
    with tab1:
        ...
        if not countries:
            ...
            if country_agg.empty:
                ...
                # ------------------------------------------------------------
                # Editable supplier-input scenario calculation
                # ------------------------------------------------------------
                st.subheader("Editable supplier-input CBAM scenario")

                scenario_year = st.selectbox(
                    "Scenario year",
                    CARBON_SCHEDULE["Year"].tolist(),
                    index=len(CARBON_SCHEDULE) - 1,
                    key=f"importer_scenario_year_{data_choice.replace(' ', '_').lower()}"
                )

                supplier_scenario = build_supplier_scenario_table(
                    importer_supplier_df,
                    annual_volume,
                    scenario_year
                )

                supplier_scenario = supplier_scenario[
                    supplier_scenario["Country"].isin(countries)
                ].copy()

                if supplier_scenario.empty:
                    st.warning("No supplier scenario rows are available for the selected countries.")
                else:
                    scenario_row = CARBON_SCHEDULE[
                        CARBON_SCHEDULE["Year"] == scenario_year
                    ].iloc[0]

                    volume_multiplier = annual_volume / BASELINE_SCENARIO_VOLUME

                    st.caption(
                        f"Scenario formula: Effective intensity × annual volume "
                        f"({annual_volume:,.0f}t) × carbon price "
                        f"(£{scenario_row['Carbon Price (£/tCO₂e)']:,.0f}) × "
                        f"phase-in factor ({scenario_row['Phase-in Factor']:.3f})."
                    )

                    k1, k2, k3, k4 = st.columns(4)
                    k1.metric("Countries selected", len(countries))
                    k2.metric(
                        "Annual volume used",
                        f"{annual_volume:,.0f} t",
                        delta=f"{volume_multiplier:.2f}× 2,500t baseline"
                    )
                    k3.metric(
                        f"Scenario CBAM {scenario_year}",
                        money(supplier_scenario["Scenario_CBAM"].sum()),
                        delta=money(supplier_scenario["Volume_Impact_vs_Baseline"].sum())
                    )
                    k4.metric(
                        f"Scenario total cost {scenario_year}",
                        money(supplier_scenario["Scenario_Total_Cost"].sum())
                    )

                    left, right = st.columns([1.25, 1])

                    with left:
                        st.subheader("Editable supplier scenario table")
                        scenario_cols = [
                            "Supplier",
                            "Country",
                            "Route",
                        ...
                        scenario_cols = [c for c in scenario_cols if c in supplier_scenario.columns]
                        st.dataframe(
                            supplier_scenario[scenario_cols].sort_values(
                                "Scenario_CBAM",
                                ascending=False
                            ),
                        ...
                        with st.expander("Workbook-based reference values"):
                            workbook_cols = [
                                "Country",
                                "Shipments",
                                "Volume",
                            ...
                            workbook_cols = [c for c in workbook_cols if c in country_agg.columns]
                            st.dataframe(country_agg[workbook_cols], use_container_width=True)

                    with right:
                        chart_mode = st.radio(
                            "Chart basis",
                            [
                                "Scenario CBAM by supplier",
                                "Scenario total cost by supplier",
                                "Baseline vs selected volume",
                                "Workbook CBAM reference"
                            ],
                            horizontal=False,
                            key=f"importer_chart_basis_{data_choice.replace(' ', '_').lower()}"
                        )

                        if chart_mode == "Scenario CBAM by supplier":
                            fig = px.bar(
                                supplier_scenario,
                                x="Supplier",
                                y="Scenario_CBAM",
                                title=(
                                    f"Scenario CBAM by supplier — "
                                    f"{scenario_year}, {annual_volume:,.0f} tonnes"
                                )
                            )
                            fig.update_layout(yaxis_title="Scenario CBAM liability (£)")

                        elif chart_mode == "Scenario total cost by supplier":
                            fig = px.bar(
                                supplier_scenario,
                                x="Supplier",
                                y="Scenario_Total_Cost",
                                title=(
                                    f"Scenario total cost by supplier — "
                                    f"{scenario_year}, {annual_volume:,.0f} tonnes"
                                )
                            )
                            fig.update_layout(yaxis_title="Scenario total cost (£)")

                        elif chart_mode == "Baseline vs selected volume":
                            chart_df = supplier_scenario[
                                ["Supplier", "Baseline_CBAM_2500t", "Scenario_CBAM"]
                            ].melt(
                                id_vars="Supplier",
                                value_vars=["Baseline_CBAM_2500t", "Scenario_CBAM"],
                                var_name="Scenario",
                                value_name="CBAM Liability"
                            )

                            chart_df["Scenario"] = chart_df["Scenario"].replace({
                                "Baseline_CBAM_2500t": f"Baseline {BASELINE_SCENARIO_VOLUME:,.0f}t",
                                "Scenario_CBAM": f"Selected {annual_volume:,.0f}t"
                            })
                            ...
                            fig = px.bar(
                                ...
                                color="Scenario",
                                barmode="group",
                                title="Baseline vs selected annual volume"
                            )
                            ...
                            fig = px.bar(
                                ...
                                x="Country",
                                y="CBAM",
                                title=f"Workbook CBAM reference — {data_choice}"
                            )
                            fig.update_layout(yaxis_title="Workbook CBAM liability (£)")
...
def render_buyer_trader_view():
    ...
    # ------------------------------------------------------------
    # Recalculate all Buyer / Trader outputs from editable inputs
    # ------------------------------------------------------------
    supplier_df["CO2 Intensity"] = pd.to_numeric(
        supplier_df["CO2 Intensity"],
        errors="coerce"
    ...
    supplier_df["Use Default Factor"] = (
        (~supplier_df["DPP Available"].astype(bool))
    ...
    supplier_df["Baseline 2500t CBAM Liability"] = (
        supplier_df["Effective Intensity for CBAM"]
        * BASELINE_SCENARIO_VOLUME
        * 100
    )

    supplier_df["Volume Impact vs 2500t"] = (
        supplier_df["2030 CBAM Liability"]
        - supplier_df["Baseline 2500t CBAM Liability"]
    )
    ...
    supplier_df["Baseline 2500t Total Cost"] = (
        supplier_df["Price per Tonne"] * BASELINE_SCENARIO_VOLUME
        + supplier_df["Baseline 2500t CBAM Liability"]
    )
    ...
    numeric_cols = [
        ...
        "Effective Intensity for CBAM",
        "2030 CBAM Liability",
        "Baseline 2500t CBAM Liability",
        "Volume Impact vs 2500t",
        "Total Cost 2030",
        "Baseline 2500t Total Cost",
        "DPP Completeness",
        "ESG Score",
    ...
    # ============================================================
    # Tab 1: Supplier Compare
    # ============================================================
    with tab1:
        st.caption(
            "This tab is recalculated from the Buyer / Trader sidebar inputs. "
            "Annual volume affects CBAM liability and total cost; price affects total cost; "
            "CO₂ intensity affects liability and ESG; recycled content, DPP and certification "
            "affect ESG and DPP risk."
        )
        ...
        k5.metric("Annual volume used", f"{annual_volume:,.0f} t")
        k6.metric(
            "Total 2030 CBAM liability",
            money(supplier_df["2030 CBAM Liability"].sum()),
            delta=money(supplier_df["Volume Impact vs 2500t"].sum())
        )
        k7.metric(
            "Total cost 2030",
            money(supplier_df["Total Cost 2030"].sum())
        )
        k8.metric(
        ...
        st.dataframe(
            supplier_df[display_cols].sort_values(
                "Total Cost 2030",
                ascending=True
            ),
            use_container_width=True
        ...
        chart_metric = st.radio(
            "Supplier comparison chart metric",
            [
                "Total Cost 2030",
                "2030 CBAM Liability",
                "ESG Score",
            ...
            horizontal=True,
            key=f"buyer_compare_metric_{data_choice.replace(' ', '_').lower()}"
        )

        fig_bar = px.bar(
            supplier_df.sort_values(chart_metric, ascending=False),
            x="Supplier",
            y=chart_metric,
            color="Recommendation",
            title=f"{chart_metric} by supplier — recalculated from Buyer / Trader inputs"
        )
        st.plotly_chart(fig_bar, use_container_width=True)

        baseline_mode = st.checkbox(
            "Show baseline 2,500t vs selected annual volume",
            value=True,
            key=f"buyer_show_baseline_{data_choice.replace(' ', '_').lower()}"
        )

        if baseline_mode:
            baseline_df = supplier_df[
                ["Supplier", "Baseline 2500t CBAM Liability", "2030 CBAM Liability"]
            ].melt(
                id_vars="Supplier",
                value_vars=[
                    "Baseline 2500t CBAM Liability",
            ...
            baseline_df["Scenario"] = baseline_df["Scenario"].replace({
                "Baseline 2500t CBAM Liability": f"Baseline {BASELINE_SCENARIO_VOLUME:,.0f}t",
                "2030 CBAM Liability": f"Selected {annual_volume:,.0f}t"
            })
            ...
            fig_base = px.bar(
                ...
                color="Scenario",
                barmode="group",
                title="Buyer volume impact: baseline 2,500t vs selected volume"
            )
    ...
    # ============================================================
    # Tab 2: ESG Risk Score
    # ============================================================
    with tab2:
        st.caption(
            "Radar dimensions are recalculated from supplier inputs: "
            "CO₂ intensity → Low Carbon; recycled content → Recycled Content; "
            "DPP and certification → DPP Quality / Certification; price → Price Value."
        )
    ...
    # ============================================================
    # Tab 3: DPP Verification
    # ============================================================
    with tab3:
        st.caption(
            "DPP verification is recalculated from DPP availability, certification, "
            "CO₂ intensity and annual volume. If DPP is unavailable or certification is Unknown, "
            "the effective CBAM intensity may switch to the default factor."
        )
        ...
        k4.metric(
            "Suppliers using default factor",
            int(supplier_df["Use Default Factor"].sum())
        )
        ...
        dpp_metric = st.radio(
            ...
            horizontal=True,
            key=f"buyer_dpp_metric_{data_choice.replace(' ', '_').lower()}"
        )

        fig = px.bar(
            supplier_df.sort_values(dpp_metric, ascending=False),
            x="Supplier",
            y=dpp_metric,
            color="DPP Available",
            title=f"{dpp_metric} by supplier — DPP verification"
        )
