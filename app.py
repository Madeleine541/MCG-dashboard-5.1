    ...
    for i, country in enumerate(countries):
        with st.sidebar.expander(f"Supplier {i + 1} — {country}", expanded=False):
            ...
            intensity = st.number_input(
                ...
                step=0.05,
                key=f"{key_prefix}_intensity_{i}"
            )
    ...
    with tab1:
        ...
        if not countries:
            ...
            if country_agg.empty:
                ...
                # ------------------------------------------------------------
                # Volume-adjusted scenario calculation
                # ------------------------------------------------------------
                st.subheader("Volume-adjusted CBAM scenario")

                scenario_year = st.selectbox(
                    "Scenario year for volume-adjusted CBAM chart",
                    CARBON_SCHEDULE["Year"].tolist(),
                    index=len(CARBON_SCHEDULE) - 1,
                    key=f"scenario_year_{data_choice.replace(' ', '_').lower()}"
                )
                ...
                country_agg["Scenario_CBAM"] = (
                    country_agg["Avg_Intensity"]
                    * annual_volume
                    * scenario_row["Carbon Price (£/tCO₂e)"]
                    * scenario_row["Phase-in Factor"]
                )

                country_agg["Scenario_TCO"] = (
                    country_agg["Scenario_CBAM"]
                    + country_agg["Price_per_t"] * annual_volume
                    if "Price_per_t" in country_agg.columns
                    else country_agg["Scenario_CBAM"]
                )

                country_agg["Scenario_CBAM_per_t"] = np.where(
                    annual_volume > 0,
                    country_agg["Scenario_CBAM"] / annual_volume,
                    0
                )
                ...
                k3.metric("Annual volume scenario", f"{annual_volume:,.0f} t")
                k4.metric(
                    f"Scenario CBAM liability {scenario_year}",
                    money(country_agg["Scenario_CBAM"].sum())
                )
                ...
                with left:
                    ...
                    st.caption(
                        "Note: `CBAM` is the workbook-based liability from the Excel records, "
                        "while `Scenario_CBAM` is recalculated using the sidebar annual volume."
                    )
                ...
                with right:
                    chart_mode = st.radio(
                        "Chart basis",
                        [
                            "Volume-adjusted scenario CBAM",
                            "Workbook CBAM liability"
                        ],
                        horizontal=False,
                        key=f"cost_chart_mode_{data_choice.replace(' ', '_').lower()}"
                    )

                    if chart_mode == "Volume-adjusted scenario CBAM":
                        y_col = "Scenario_CBAM"
                        chart_title = (
                            f"Volume-adjusted CBAM liability by country — "
                            f"{scenario_year}, {annual_volume:,.0f} tonnes"
                        )
                    else:
                        y_col = "CBAM"
                        chart_title = f"Workbook country-level CBAM liability — {data_choice}"
                ...
                cleanest = country_agg.sort_values("Avg_Intensity").iloc[0]
                dirtiest = country_agg.sort_values("Avg_Intensity", ascending=False).iloc[0]

                saving_5y = 0

                for _, row in CARBON_SCHEDULE.iterrows():
                    saving_5y += (
                        (dirtiest["Avg_Intensity"] - cleanest["Avg_Intensity"])
                        * annual_volume
        ...
        if forecast.empty:
            ...
            with c1:
                fig = px.line(
                    ...
                    color="Supplier",
                    markers=True,
                    title="Projected CBAM liability by supplier"
                )
                st.plotly_chart(fig, use_container_width=True)
            ...
            with c2:
                cumulative = (
                    forecast
                    .groupby("Supplier", as_index=False)["CBAM Liability"]
                    .sum()
                    .sort_values("CBAM Liability")
                )
                ...
                fig2 = px.bar(
                    ...
                    x="Supplier",
                    y="CBAM Liability",
                    title="Cumulative 2025–2030 CBAM liability"
                )
...
def render_manufacturer_view():
    ...
    with tab3:
        ...
        fig = px.line(
            f,
            x="Year",
            y=["CBAM Liability", "Sector average liability"],
            markers=True,
            title="Buyer CBAM burden: your steel vs sector average"
        )
...
st.caption(
    "Model note: the dashboard separates the CBAM case dataset and the Simulated Data "
    "portfolio dataset. Workbook consignment records drive the cost calculator, while "
    "DPP, certification, recycled-content and manufacturer inputs remain scenario inputs "
    "because those fields are not available as complete supplier-level records in the workbook."
)
